...
BINARY_CONTAINERS += $(BUILD_DIR)/SW.xclbin
BINARY_CONTAINER_SW_OBJS += $(TEMP_DIR)/SW.xo
...
# Building kernel
$(TEMP_DIR)/SW.xo: src/wapa.cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) $(CLFLAGS) --temp_dir $(TEMP_DIR) -c -k SW -I'$(<D)' -o'$@' '$<'
$(BUILD_DIR)/SW.xclbin: $(BINARY_CONTAINER_SW_OBJS)
	mkdir -p $(BUILD_DIR)
...
ifeq ($(HOST_ARCH), x86)
	...
	XCL_EMULATION_MODE=$(TARGET) ./$(EXECUTABLE) $(BUILD_DIR)/SW.xclbin
else
	mkdir -p $(EMU_DIR)
...
ifeq ($(HOST_ARCH), x86)
	./$(EXECUTABLE) $(BUILD_DIR)/SW.xclbin
endif
endif
